...
We call helpers a set of custom bash function created by the YunoHost project to standardize common operations accross all apps. They are all prefixed with `ynh_`. The full list and documentation of these helpers is available on [this page](/contribute/packaging_apps/scripts/packaging_app_scripts_helpers_v21). Some of these helpers are now partially obsolete as they are now handled by the core via app resources.
...
More infos on the `source` resource in [the resource system documentation](/contribute/packaging_apps/manifest/app_resources).
