...
Per alcune applicazioni è possibile installarne più copie (in diverse directory) ! Per fare ciò è sufficiente cliccare su <HighlightApplications/> → <HighlightAppInstall/> e selezionare nuovamente l'applicazione da installare.
...
È possibile accedere alla pagina relative alle operazioni dell'applicazione cliccando sul bottone <Highlight no_break={true} color='darkblue'><FAIcon icon="fa-cogs"/> Config panel</Highlight>.
