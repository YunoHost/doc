...
C'est le backend de connexion unique (single sign-on) de YunoHost. Il est basé sur des stripts Lua interfacés directement avec NGINX
et qui gère tous les aspects "techniques" de l'authentification et de la gestion des accès aux ressources.
...
Ce portail est utilisé par les utilisateurs pour se connecter et lister les applications installées.
