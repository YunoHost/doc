...
4. La clé `routes` et son enfant `default` font que la page est accessible par défaut à l'adresse `https://yunohost.org/docs/doc` au lieu de devoir la chercher à l'adresse `https://yunohost.org/docs/dev/doc`, qui correspond à son emplacement réel dans la hiérarchie des dossiers.

## Syntaxe

Vous pouvez utiliser la syntaxe Markdown, consultez la page de [documentation dédiée](/dev/doc/markdown_guide) pour plus d'information.
