...
- Assurez-vous d'avoir [configuré les redirections de ports](/admin/get_started/post_install/port_forwarding)
...
- Si votre serveur est chez vous, vous avez sans doute accès au réseau local du serveur. Depuis ce réseau, vous pouvez vous connecter [en SSH](/admin/command_line) avec l'utilisateur `root`.
...
- son IP locale, en utilisant l'une des méthodes [listées sur la page correspondante](/admin/get_started/post_install/find_ip)
- via `yunohost.local`, si votre client support le Bonjour protocol (ou `yunohost-2.local`, etc. en fonction de si vous avez plusieurs serveurs YunoHost sur le même réseau)
