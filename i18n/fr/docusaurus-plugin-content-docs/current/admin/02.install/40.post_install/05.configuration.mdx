...
[Trouvez son adresse IP](/admin/install/post_install/find_ip) avant de continuer.
...
- Si en revanche vous avez déjà votre propre nom de domaine, vous souhaitez probablement l'utiliser. Vous aurez donc besoin ensuite de configurer les enregistrements DNS comme expliqué [ici](/admin/install/post_install/dns_config).
...
:::tip
N.B. : **ne partez pas en courant** ! La première fois que vous lancerez le diagnostic, il est assez normal d'avoir plusieurs alertes rouges ou jaunes car vous devez généralement [configurer les enregistrements DNS](/admin/install/post_install/dns_config) (si vous n'utilisez pas un domaine `.nohost.me`, `.noho.st` ou `.ynh.fr`), ajouter un fichier de swap ainsi que [configurer la redirection des ports](/admin/install/post_install/port_forwarding) sur votre box.
:::
