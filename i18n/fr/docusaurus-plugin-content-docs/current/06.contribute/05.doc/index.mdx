...
{/* TODO: Migrer cette documentation pour Docusaurus */}
...
4. La clé `routes` et son enfant `default` font que la page est accessible par défaut à l'adresse `https://yunohost.org/docs/doc` au lieu de devoir la chercher à l'adresse `https://yunohost.org/docs/contribute/doc`, qui correspond à son emplacement réel dans la hiérarchie des dossiers.

## Syntaxe

Vous pouvez utiliser la syntaxe Markdown, consultez la page de [documentation dédiée](/contribute/doc/markdown_guide) pour plus d'information.
...
| Page          | Chemin | Notes |
|---------------|--------|-------|
| Catalogue d'applications  | `/pages/02.applications/01.catalog/apps.md` | Récupère et traite le fichier [app.json](https://github.com/YunoHost/apps/blob/master/apps.toml) |
