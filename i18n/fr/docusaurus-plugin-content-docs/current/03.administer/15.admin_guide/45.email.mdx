...
Pour interagir avec le serveur de mail, c'est-à-dire lire et envoyer des emails, vous pouvez soit installer un client web comme Roundcube ou Rainloop sur votre serveur, soit configurer un client de bureau ou mobile comme décrit dans [cette page](/user_guide/email_configure_client).
...
Les groupes aussi peuvent utiliser des alias, par défaut le groupe `admins` dispose de `root@<domain.tld>` ou encore `webmaster@<domain.tld>`. [Plus d'information dans la page dédiée](/administer/admin_guide/users/groups_and_permissions#gérer-les-alias-des-groupes).
...
Pour s’assurer que l’adresse IP de votre serveur n’est pas dans ces listes et pour l’enlever dans le cas échéant suivez ce [lien](/administer/troubleshooting/blacklist_forms).

## Migration des emails d'un fournisseur d'emails vers une instance de YunoHost

Voir [cette page](/user_guide/email_migration).

## Configuration du relais SMTP

Voir [cette page](/administer/tutorials/email_configure_relay).
