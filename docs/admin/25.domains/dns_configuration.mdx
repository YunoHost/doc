...
Note YunoHost supports the [automatic configuration of DNS record](/admin/domains/#dns-configuration) for some registrar.
...
## A brief explication of DNS records.
...
- Reminder that a domain specified in the data of the record **need to ends with a dot** (to be a FQDN as explained previously).

## Minimal DNS configuration

When registering a custom domain name through YunoHost interface, this is the minimal
