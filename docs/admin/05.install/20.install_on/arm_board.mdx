...
- A [reasonable ISP](/admin/self_hosting/providers/isp/), preferably with a good and unlimited upstream bandwidth
...
Next you need to [find the local IP address of your server](/admin/install/post_install/find_ip) to connect as root user [via SSH](/admin/command_line) with the temporary password `1234` (see [the Armbian doc](https://docs.armbian.com/User-Guide_Getting-Started/#how-to-login)).

```bash
ssh root@192.168.x.xxx
```

Now you can follow the generic instructions to [install YunoHost on Debian](/admin/install/install_on/on_top_of_debian#-run-the-install-script).
