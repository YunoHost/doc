...
2. Since YunoHost 11.2, you can now define a password on your nohost.me, noho.st or ynh.fr domain, so that the recovery is automatic and no longer requires to post for registration removal [in the dedicated forum thread](https://forum.yunohost.org/t/nohost-domain-recovery/442).
