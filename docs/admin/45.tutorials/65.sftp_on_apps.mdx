...
- `/var/www/wordpress/wp-config.php`  must be remplaced by the path your app config file. eg : for yeswiki (/var/www/yeswiki/wakka.config.php)
...
```
quick commands explaination :
- `mkdir -p /PATH`create a folder in the specified PATH
...
- `setfacl` set the file permission
