...
pagination_prev: admin/install/install_on/index
---
...
[Find out its IP address](/admin/install/post_install/find_ip) before continuing.
...
- If you already own a domain name, you probably want to use it here. You will later need to configure DNS records as explained [here](/admin/install/post_install/dns_config).
...
:::tip
**Don't run away** ! The first time you run the diagnosis, it is quite expected to see a bunch of yellow/red alerts because you typically need to [configure DNS records](/admin/install/post_install/dns_config) (if not using a `.nohost.me`/`noho.st`/`ynh.fr` domain), add a swapfile if not enough ram as well as [port forwarding](/admin/install/post_install/port_forwarding) on your home's router.
:::
...
You now have a pretty well configured server. If you're new to YunoHost, we recommend to have a look at [the guided tour](/admin/webadmin). You should also be able to [install your favourite applications](https://apps.yunohost.org/). Don't forget to [plan backups](/category/backups) !
