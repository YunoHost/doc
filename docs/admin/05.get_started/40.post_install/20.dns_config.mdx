...
## A brief explication of domain names 

Because understanding domains can be tricky it might be worth detailing how they work.

A domain is an human readable address made out of *labels* concatenated by dots.
...
talk about *subdomain* for any domain that have 2 levels after the top-level domain.
...
when not, is represented by a single dot at the end. This form is referred as 
"fully qualified domain name" (FQDN). With our previous example, `www.yunohost.org.` would be a FQDN. 
This is good to remember because you usually need to specify a FQDN in your DNS records.

And still for DNS records, the `@` name will denote the current origin which should be
you apex domain as FQDN (with our previous example `@` = `yunohost.org.`).


## Recommended DNS configuration

:::note
For example purposes we use the placeholder domain `your.domain.tld`, you have to replace it with your real domain, such as `yunohost.org`.
:::
...
Though it might be easier to understand if displayed as a table instead:
...
If you are looking to configure a subdomain, [follow this tutorial](/admin/tutorials/domains/dns_subdomains).
