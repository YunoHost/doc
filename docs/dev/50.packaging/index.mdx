...
It might be tempting to use an LLM to generate app packages. Such "contributions" have systematically brought too verbose code and comments, helpers hallucinations, non-standard directory architectures and files. Reviewing such packages is tiresome for the contributors. It has been decided to reject generated packages that do not follow the `example_ynh` template app.
:::
