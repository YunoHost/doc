...
- `license` (`str`) : the license code of the *upstream* project. (Note that only apps based on free software will be accepted in the official YunoHost app catalog.). The license code should be [a valid SPDX identified](https://spdx.org/licenses/). In case there is no corresponding license in SPDX's list, then you can use the most appropriate short name/acronym you can think of for the given license and link to the license file using the `license_url` property documented below.
