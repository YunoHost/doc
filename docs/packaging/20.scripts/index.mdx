...
We call helpers a set of custom bash function created by the YunoHost project to standardize common operations accross all apps. They are all prefixed with `ynh_`. The full list and documentation of these helpers is available on [this page](/packaging/scripts/helpers_v2.1). Some of these helpers are now partially obsolete as they are now handled by the core via app resources.
...
More infos on the `source` resource in [the resource system documentation](/packaging/manifest/resources).
